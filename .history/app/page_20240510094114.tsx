...
export default function Home() {
   ...
   useEffect(()=>{
    ...
    useEffect(()=>{
        ...
        if(sref.current && initDone){
            const options = {
                // root: null, // 视口的根元素，默认为整个文档视口
                //rootMargin: '0px',
                threshold: 0.8 // 目标元素可见度的阈值，默认为 0（完全不可见）到 1（完全可见）
            };
            const observer = new IntersectionObserver(()=>{
                
                const spline = splineRef.current;
                if (spline) {
                    console.log('---inininin---',spline);
                    // spline.play();
                    // spline.render()
                    //spline.emitEvent('mouseHover', 'Cube','onStart', 'onWheel');
                }
            }, options);
        ...
        <main className="bg-block h-fit w-full flex-1 absolute top-0 box-border bg-[#100F22] relative selector">
            <div className="mx-auto w-full lg:h-screen sm:h-banner bg-cover relative ">
                ...
                <div className="lg:w-1/2 lg:h-24 md:h-96 m-auto text-center absolute sm:top-96 lg:top-0 bottom-0 left-0 right-0">
                    <h1 ref={typedel} id="tttt1" className="lg:text-8xl md:text-6xl sm:text-5xl sm:mt-24 font-extrabold text-write-900"></h1>
                    <h1 ref={typedel} id="tttt2" className="lg:text-8xl md:text-6xl sm:text-5xl sm:mt-24 font-extrabold text-write-900"></h1>
            ...
            {/* part 1 */}
                <div className="mx-auto w-full lg:h-screen sm:h-auto bg-cover flex lg:flex-row sm:flex-col-reverse sm:text-center items-center lg:px-28 sm:px-mspace relative">
                    ...
                    <div id="tt" ref={sref} className="lg:w-120 lg:basis-2/5">
                        <Spline  onLoad={onLoad} className="w-full h-full"  scene="https://prod.spline.design/iCjOQX3y10yB57-y/scene.splinecode" />
                    </div>
                </div>
            {/* part 2 */}
