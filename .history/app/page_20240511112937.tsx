...
export default function Home() {
   ...
   useLayoutEffect(()=>{
    ...
    useEffect(() => {
        ...
        if(fullref.current){
           // fullref.current.setAllowScrolling(true);
            console.log('----fullref----',fullref.current)
            const y = fullref.current.getScrollY();
            console.log('----fullref-y---',fullref.current)
        }
    },[fullref])
  return (
    <>
     { initDone &&
        <main className="bg-block h-fit w-full flex-1 absolute top-0 box-border bg-[#100F22] relative selector">
            <div className="section mx-auto w-full lg:h-screen sm:h-banner bg-cover relative z-1">
                <Suspense>
            ...
            <ReactFullpage
                ...
                render={({ state, fullpageApi }) => {
                    fullref.current = fullpageApi;
                    return (
                        <ReactFullpage.Wrapper>
                        {/* part 1 */}
                            <div className="section bg-[#100F22] mx-auto w-full lg:h-screen sm:h-auto bg-cover flex lg:flex-row sm:flex-col-reverse sm:text-center items-center lg:px-28 lg:gap-x-14 sm:px-mspace relative">
